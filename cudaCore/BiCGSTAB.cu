...
__global__ void computeX(Scalar* x,Scalar* p,int length,double alpha)
{
	auto id = threadIdx.x + blockIdx.x * blockDim.x;
	if(id >= length)
		return;
...
void BiCGSTAB(const SymetrixSparseMatrix& A,Vector& x,const Vector& b,double tolerance,int limit,int& iter,double& norm)
{
	...
	while(iter < limit && norm > tolerance * normb)
	{
		...
		computeX<<<blockSize,threadSize>>>(thrust::raw_pointer_cast(&xx[0]),thrust::raw_pointer_cast(&p[0]),x.size(),alpha);
		...
		normb = thrust::reduce(thrust::device,temp.begin(),temp.end());
